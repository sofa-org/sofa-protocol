...
// File contracts/tokenomics/RBTC.sol
...
contract RBTC is IERC20, Ownable {
    ...
    event Deposited(address indexed account, uint256 amount, uint256 sharesAmount);
    event Withdrawn(address indexed account, uint256 amount, uint256 sharesAmount);
    event CollateralRebased(uint256 oldTotalCollateral, uint256 newTotalCollateral);
    ...
    function deposit() external payable {
        ...
        uint256 sharesAmount = totalSupply() == 0
            ? msg.value
            : getSharesByPooledBTC(msg.value);
        _totalCollateral += msg.value;
        _mintShares(msg.sender, sharesAmount);

        emit Deposited(msg.sender, msg.value, sharesAmount);
        _emitTransferAfterMintingShares(msg.sender, sharesAmount);
    }

    function withdraw(uint256 amount) external {
        require(amount <= address(this).balance, "INSUFFICIENT_COLLATERAL_BALANCE");
        require(amount <= balanceOf(msg.sender), "INSUFFICIENT_RBTC_BALANCE");

        uint256 sharesAmount = getSharesByPooledBTC(amount);
    ...
    function rebaseCollateral(uint256 newTotalCollateral) external onlyOwner {
        _totalCollateral = newTotalCollateral;
        emit CollateralRebased(_totalCollateral, newTotalCollateral);
    }

    function transferCollateral(address to, uint256 amount) external onlyOwner {
        require(amount <= address(this).balance, "RBTC: Insufficient balance");
     ...
     *
     * @dev The sum of all BTC balances in the protocol, equals to the total supply of rBTC.
     */
     ...
     */
    function _getTotalPooledBTC() internal view returns (uint256) {
     ...
     * - `_recipient` cannot be the zero address or the `rBTC` token contract itself
    ...
    function _transferShares(address _sender, address _recipient, uint256 _sharesAmount) internal {
        ...
        require(_recipient != address(this), "TRANSFER_TO_RBTC_CONTRACT");
    ...
    function _burnShares(address _account, uint256 _sharesAmount) internal returns (uint256 newTotalShares) {
        ...
        uint256 preRebaseTokenAmount = getPooledBTCByShares(_sharesAmount);
    ...
    function _emitTransferAfterMintingShares(address _to, uint256 _sharesAmount) internal {
        _emitTransferEvents(address(0), _to, getPooledBTCByShares(_sharesAmount), _sharesAmount);
    }
}
