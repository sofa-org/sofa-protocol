...
contract SimpleSmartTrendVault is Initializable, ContextUpgradeable, ERC1155Upgradeable, ReentrancyGuardUpgradeable, SignatureBitMap {
    ...
    function _burn(uint256 expiry, uint256[2] memory anchorPrices, uint256 isMaker) internal nonReentrant returns (uint256 payoff) {
        ...
        if (isMaker == 1) {
            ...
            payoff = getMinterPayoff(expiry, anchorPrices, amount);
        }
    ...
    function _burnBatch(Product[] calldata products) internal nonReentrant returns (uint256 totalPayoff) {
        ...
        for (uint256 i = 0; i < products.length; i++) {
            ...
            if (product.isMaker == 1) {
                ...
                payoffs[i] = getMinterPayoff(product.expiry, product.anchorPrices, amount);
            }
            if (payoffs[i] > 0) {
    ...
    function getMinterPayoff(uint256 expiry, uint256[2] memory anchorPrices, uint256 amount) public view returns (uint256 payoff) {
        payoff = strategy.getMinterPayoff(anchorPrices, oracle.settlePrices(expiry), amount);
    }
